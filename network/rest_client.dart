...
class RestClient {
  ...
  static Future<http.Response> get(Uri url,
      {Map<String, String>? headers, bool silentMode = false}) async {
    if (!silentMode) {
  ...
  static Future<http.Response> post(Uri url,
      {Map<String, String>? headers, Object? body, bool silentMode = false}) {
    if (!silentMode) {
      PaperTrailClient.sendInfoMessageToPaperTrail(
  ...
  static Future<http.Response> multipartFileRequest(Uri url,
      Map<String, String> fields, Map<String, List<int>> files, String filename,
      {Map<String, String>? headers, bool silentMode = false}) {
    if (!silentMode) {
      PaperTrailClient.sendInfoMessageToPaperTrail(
    ...
    files.forEach((String key, List<int> value) {
      request.files
          .add(http.MultipartFile.fromBytes(key, value, filename: filename));
    });
